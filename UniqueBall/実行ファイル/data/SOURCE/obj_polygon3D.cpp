...
HRESULT CObject3D::Init(void)
{
	...
	//頂点座標の設定
	pVtx[0].pos = D3DXVECTOR3(-DEF_SIZE.x, 0.0f, DEF_SIZE.y);		//(x,y,z)
	pVtx[1].pos = D3DXVECTOR3(DEF_SIZE.x, 0.0f, DEF_SIZE.y);
	pVtx[2].pos = D3DXVECTOR3(-DEF_SIZE.x, 0.0f, -DEF_SIZE.y);
	pVtx[3].pos = D3DXVECTOR3(DEF_SIZE.x, 0.0f, -DEF_SIZE.y);
...
void CObject3D::GetPolyPos(D3DXVECTOR3 *Vtx0, D3DXVECTOR3 *Vtx1, D3DXVECTOR3 *Vtx2, D3DXVECTOR3 *Vtx3)
{
	...
	if (Vtx1 != nullptr)*Vtx1 = pVtx[1].pos;
	if (Vtx2 != nullptr)*Vtx2 = pVtx[2].pos;
	if (Vtx3 != nullptr)*Vtx3 = pVtx[3].pos;
...
// 法線設定
//============================
void CObject3D::SetNormal(const D3DXVECTOR3 nor)
{
	VERTEX_3D* pVtx;
...
// 描画時レンダラー設定
//==========================================
void CObject3D::DrawRenderSet(LPDIRECT3DDEVICE9 pDevice)
{
	//Zテストを無効にする
	if (m_bZtest == true)
	{
...
// 描画時レンダラー設定解除
//==========================================
void CObject3D::DrawRenderReset(LPDIRECT3DDEVICE9 pDevice)
{
	//ライティングをTRUEに設定
	if (m_bLight)
	{
...
HRESULT CObject3DAnim::Init(const D3DXVECTOR3 pos, const D3DXVECTOR3 rot)
{
	...
	CObject3D::SetSize(D3DXVECTOR3(DEF_SIZE_ANIM.x, 0.0f, DEF_SIZE_ANIM.y));
...
void CObject3DAnim::Update(void)
{
	...
	{
		pVtx[nCntTex].tex = pTexpos[nCntTex];
	}
...
// 2Dポリゴンの生成処理
//============================
CObject3DAnim* CObject3DAnim::Create(void)
{
	CObject3DAnim* pObject3D;

	//オブジェクト生成
	pObject3D = new CObject3DAnim;
...
// 3Dポリゴンの生成処理
//============================
CObject3DAnim* CObject3DAnim::Create(const D3DXVECTOR3 pos, const D3DXVECTOR3 rot, const D3DXVECTOR3 size)
{
