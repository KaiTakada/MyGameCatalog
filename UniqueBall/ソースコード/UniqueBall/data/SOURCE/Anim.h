...
public:
	...
	void SetNumPtnXY(const int nNumPtnX, const int nNumPtnY) { SetNumPtnX(nNumPtnX),SetNumPtnY(nNumPtnY); }			//パターン数設定xy
	...
	D3DXVECTOR2* GetTexpos(void) { return m_aTexpos; }		//テクスチャ座標取得
	D3DXVECTOR2 GetTexpos(int nIdx) { return m_aTexpos[nIdx]; }		//テクスチャ座標取得
...
private:
	...
	int m_nPtnAnimX;		//アニメーションパターンx 現在No.
	int m_nPtnAnimY;		//アニメーションパターンy 現在No.
